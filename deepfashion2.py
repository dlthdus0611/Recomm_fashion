...
os.chdir('C:\\Users\\soyeon\\Desktop\\conference')
...
# DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR, "checkpoints")
...
class DeepFashion2Config(Config):
    ...
    valid_json_path = "./dataset/validation.json"
...
def save_cropped(model, image_path):
    if image_path:
        # Run model detection and generate the color splash effect
        print("Running on {}".format(args.image))
        # Read image
...
def show_detect(model, image_path):
    if image_path:
        # Run model detection and generate the color splash effect
        print("Running on {}".format(args.image))
        # Read image
...
if __name__ == "__main__":
    ROOT_DIR = os.path.abspath("./")
    DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR, "../../PycharmProjects/Recomm_fashion/logs")
    COCO_WEIGHTS_PATH = os.path.join(ROOT_DIR, "mask_rcnn_coco.h5")
    import argparse
    ...
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash' or 'showing' or 'crop'")
    ...
    if args.command == "train":
        ...
        show_detect(model, image_path=args.image)

    elif args.command == 'crop':
        save_cropped(model, image_path=args.image)
    else:
